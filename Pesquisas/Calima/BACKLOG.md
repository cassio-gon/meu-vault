...
| **Bug** — `window.open()` chamado depois de um `await` de decifração na aba Documentos do portal (`public/js/portal/portal-documentos.js:26,32`, `public/js/documentos-paciente-ui.js:67`, commits `9729ebe`/`77a9c02` de 08/09) — no Safari/iOS a ativação do clique tende a não sobreviver à espera assíncrona, o pop-up é bloqueado e o app mostra "Permita pop-ups" (diagnóstico errado); o repo já sabia dessa classe de problema em `receita-folha.js:5` (PWA standalone no iOS) mas o padrão voltou aqui. Conserto: abrir a aba em branco de forma síncrona no clique e só setar `.location` depois de decifrar | 2026-09-09 | aberto |
| Unificar `abrirPayload` (decifra-e-abre) num único helper em `documentos-cifra.js` — hoje duplicado quase idêntico entre `documentos-paciente-ui.js` e `portal-documentos.js` (ambos de 08/09), o que fez o bug do pop-up acima se repetir nos dois lugares | 2026-09-09 | aberto |
| Foco automático no campo de senha de `pedirSenha()` (`public/js/portal/portal-documentos.js:74`) — paciente precisa tocar no campo antes de digitar, após reload | 2026-09-09 | aberto |
| Integração — `navigator.share()` como alternativa ao `window.open` para abrir/compartilhar o documento decifrado da aba Documentos no celular (o comentário do código já mira esse caso de uso: salvar em Arquivos ou mandar no WhatsApp); testar se sofre a mesma restrição de ativação do achado acima | 2026-09-09 | aberto |
| **Governança** — Passe de risco Fable 5.1 retroativo no commit `d051a79` (15/09) — abre `PUT`/`DELETE` contra a "Agenda pacientes" do Google (`server/routes/agenda.js:444-485`) com credencial de service account, mas saiu com Opus; é o único commit da semana tocando credencial externa que furou o gate do `CLAUDE.md`. Rodar antes de ligar `GOOGLE_SA_JSON_B64`/`GOOGLE_CALENDAR_ID` em produção | 2026-09-16 | aberto |
